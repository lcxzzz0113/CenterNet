...
def plot_loss_curve(log_file):
    with open(log_file) as f:
        ...
        for i in range(len(all_lines)):
            total_loss1 = all_lines[i].split(' ')[4]
            hm_loss1 = all_lines[i].split(' ')[7]
            wh_loss1 = all_lines[i].split(' ')[10]
            off_loss1 = all_lines[i].split(' ')[13]
            time1 = all_lines[i].split(' ')[16]
...
if __name__ == '__main__':
    ...
    ax.plot(range(len(loss_res18)), loss_res18, 'c', label='res_18_train_loss', linewidth=1)
    ...
    ax.set_title('Loss of CenterNet')
    ax.legend(loc='best')   #将图例摆放在不遮挡图线的位置
    ax.grid()
